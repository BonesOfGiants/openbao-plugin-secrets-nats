...
func TestBackend_OperatorJwt_Read(t *testing.T) {
	t.Run("old-style claims", func(_t *testing.T) {
		t := testBackend(_t)
		...
		claimsJson, err := json.Marshal(testClaims)
		...
		req := &logical.Request{
			...
			Data: map[string]any{
				...
				"claims":                unmarshalToMap(claimsJson),
			},
		}
	...
	t.Run("claims", func(_t *testing.T) {
		t := testBackend(_t)

		testClaims := &jwt.Operator{
		...
		req := &logical.Request{
			...
			Data: map[string]any{
				...
				"claims":                fromOperatorClaims(testClaims),
			},
		}
		resp, err := t.HandleRequest(context.Background(), req)
		RequireNoRespError(t, resp, err)
		...
		// operator config
		assert.Equal(t, testClaims.SigningKeys, opClaims.Operator.SigningKeys)
		assert.Equal(t, testClaims.AccountServerURL, opClaims.Operator.AccountServerURL)
		assert.Equal(t, testClaims.OperatorServiceURLs, opClaims.Operator.OperatorServiceURLs)
		assert.Equal(t, testClaims.AssertServerVersion, opClaims.Operator.AssertServerVersion)
		assert.Equal(t, testClaims.StrictSigningKeyUsage, opClaims.Operator.StrictSigningKeyUsage)
...
func TestBackend_AccountJwt_Read(t *testing.T) {
	...
	t.Run("claims", func(_t *testing.T) {
		...
		claims.SigningKeys.Add("AD7NGPOM42ASLFM3BLMU3DEPVJ23CZUQEZSCLCIK7CE6W4UOQJCKWV44")

		id := AccountId("op1", "acc1")
		SetupTestAccount(b, id, map[string]any{
			"claims": fromAccountClaims(claims),
		})
		...
		// account config
		assert.Equal(b, claims.Imports, accClaims.Account.Imports)
		assert.Equal(b, claims.Exports, accClaims.Account.Exports)
		assert.Equal(b, claims.Limits, accClaims.Account.Limits)
		assert.Equal(b, claims.SigningKeys, accClaims.Account.SigningKeys)
		assert.Equal(b, claims.Revocations, accClaims.Account.Revocations)
		assert.Equal(b, claims.DefaultPermissions, accClaims.Account.DefaultPermissions)
		assert.Equal(b, claims.Mappings, accClaims.Account.Mappings)
		assert.Equal(b, claims.Authorization, accClaims.Account.Authorization)
		assert.Equal(b, claims.Trace, accClaims.Account.Trace)
		assert.Equal(b, claims.ClusterTraffic, accClaims.Account.ClusterTraffic)
		assert.Equal(b, claims.Info.Description, accClaims.Account.Info.Description)
